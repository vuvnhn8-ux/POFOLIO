(() => {
  ...
  const short = (v) => (v >= 1000 ? `${(v / 1000).toFixed(1)}k` : String(v));
  ...
  const SOURCE_CLASS = { direct: 'direct', google: 'google', linkedin: 'linkedin', facebook: 'facebook' };
  ...
  function render(d) {
    const set = (k, v) => { const node = $(`[data-k="${k}"]`); if (node) node.textContent = fmt(v); };
    ...
    renderList('topPages', d.topPages, (r) => `<tr><td>${esc(r.label)}</td><td>${fmt(r.value)}</td></tr>`);
    renderList('topCountries', d.topCountries, (r) => `<tr><td>${esc(r.label)}</td><td>${fmt(r.value)}</td></tr>`);
    renderList('topCities', d.topCities, (r) => `<tr><td>${esc(r.label)}</td><td>${fmt(r.value)}</td></tr>`);
    ...
    drawBars($('[data-chart="visitors"]'), d.timeseries.map((t) => ({ label: t.date, value: t.visitors })), { day: true, showEvery: Math.ceil(d.timeseries.length / 8) });
    drawBars($('[data-chart="pageviews"]'), d.timeseries.map((t) => ({ label: t.date, value: t.pageviews })), { day: true, showEvery: Math.ceil(d.timeseries.length / 8) });
    drawBars($('[data-chart="geo"]'), d.topCountries.slice(0, 8));
    drawDonut($('[data-chart="sources"]'), d.sources, 'sources');
    drawDonut($('[data-chart="devices"]'), d.devices, 'devices');
    drawBars($('[data-chart="browsers"]'), d.browsers.slice(0, 6));
    drawBars($('[data-chart="os"]'), d.os.slice(0, 6));
  }

  function renderList(name, rows, rowHtml) {
  ...
  function emptyState(ctx, cssW, cssH) {
  ...
  function drawDonut(canvas, data, legendKey) {
    const s = setupCanvas(canvas);
    if (!s) return;
    const { ctx, cssW, cssH } = s;
